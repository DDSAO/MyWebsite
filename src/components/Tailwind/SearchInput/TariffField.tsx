...
export const TariffField = (props: {
  onChangeF: (code: Tariff | null) => void;
  value: Tariff | null;
  title?: string;
  disableEdit?: boolean;
}) => {
            ...
            {options.map((option, i) => {
              return (
                <li
                  ...
                  className={`cursor-pointer flex flex-row items-center justify-between text-md text-left m-1 p-2 rounded-md hover:bg-slate-300 ${
                    value && option.serialNumber === value.serialNumber
                      ? "bg-slate-200"
                      : ""
                  } overflow-hidden`}
                >
                  <p className="text-md whitespace-nowrap">{option.name} </p>
