...
export const PortCodeField = (props: {
  onChangeF: (code: PortCode | null) => void;
  value: PortCode | null;
  title?: string;
  disableEdit?: boolean;
}) => {
  ...
  const [options, setOptions] = useState<PortCode[]>([]);
        ...
        >
          {open ? (
            ...
            <div className="absolute left-0 w-full h-8">
              ...
              >
                {value && value.code ? value.code : "NOT SELECTED"}
              </p>
            </div>
          )}
            ...
            {options.map((option, i) => {
              return (
                <li
                  ...
                  onClick={() => {
                    onChangeF(option);
                    setInputValue(option.code ? option.code : "");
                    setOpen(false);
                  }}
                  className={`cursor-pointer flex flex-col items-start justify-start text-md text-left m-1 p-2 rounded-md hover:bg-slate-300 ${
                    value && option.code === value.code ? "bg-slate-200" : ""
                  } overflow-hidden`}
                >
                  <b className="text-xs whitespace-nowrap">{option.code} </b>
