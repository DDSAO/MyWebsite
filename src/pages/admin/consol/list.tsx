...
import {
  Consol,
  ConsolFilters,
  EMPTY_CONSOL_FILTERS,
} from "@/interfaces/consolDefinitions";
...
import { timestampToDateStr, toTimestamp } from "@/lib/utils";
...
import { ConsolRow } from "@/components/app/admin/consol/list/ConsolRow";
...
export const SelectedConsolAtom = atom<string | null>({
  key: "/consol/list/SelectedConsolAtom",
  default: null,
});
...
export default function Page({
  ...
  const search = usePost<
    { filters: ConsolFilters },
    { consols: Consol[]; totalNum: number }
  >({
    url: "/consol/search",
  });
  ...
  return (
    <PageWithNav user={user}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        ...
        <div className="lg:col-span-3 rounded-lg shadow-lg border gap-4 p-4">
          <BasicTable
            loading={search.loading}
            buttons={
            ...
            rows={(search.data?.consols ?? []).map((consol, i) => {
              return <ConsolRow key={i} consol={consol} />;
            })}
            totalNum={search.data?.totalNum ?? 0}
            onPageChangeF={(page_no, page_num) => {
              console.log(page_no, page_num);
              setFilters({
                ...filters,
                page_no,
                page_num,
